...
| Owner | Objects / jobs |
|---|---|
| **Admin / OLM** | CRD, operator Deployment/SA/RBAC, `CliMcpInstance` CR, investigation kubeconfig Secret `cli-mcp-<name>-kubeconfig`, TLS Secret `cli-mcp-<name>-tls` on generic Kubernetes, OpenShift SCC / PSA as needed. Admin mints a TokenRequest against `status.clientServiceAccount` (`kubectl create token` / `oc create token`). Admin does **not** apply an `/mcp` ClusterRole, a shared `cli-mcp-client` SA, or a per-instance auth-delegator CRB. |
| **Operator** | MCP Deployment+Service, MCP SA, Role/RoleBinding (pods + secret create/delete), sandbox SA (Q12), client SA `cli-mcp-<name>-client` + Role/RoleBinding (`climcpinstances/mcp`), ConfigMap `cli-mcp-<name>-krp`, NetworkPolicies (Q11), serving-cert annotation when on OpenShift, HMAC generate-once, ClusterRoleBinding `cli-mcp-auth-delegator` (Q13). **namespaced Role** (`config/rbac/namespaced_role.yaml`) including secrets and configmaps (HMAC, Ready, idle GC/finalizer, krp ConfigMap) — OperatorGroup target namespace is the **secret trust boundary**. ClusterRole `manager-role`: CR watch plus `bind` on `system:auth-delegator`, unscoped CRB `create`, and named `cli-mcp-auth-delegator` get/update/patch (no `list`/`watch`/`delete`; Get by name). Warm pool, idle GC, teardown (Q10). Later: proxy children. |
...
**Decision:** Option A — this hybrid. Instance infrastructure + HMAC generate-once + warm pool + idle GC + instance teardown + per-instance MCP client identity (Q13) on the operator. MCP keeps claim / on-demand create / exec / explicit session delete. No session CRD. Investigation kubeconfig stays admin-provided. Operator does **not** mint investigation tokens or MCP client tokens.

_Considered and rejected: MCP owns pool and idle GC as well (replica stampede; janitor dies with the Deployment), `Sandbox` CRD (MCP apply CR, operator creates pods — extra hop, second API, HMAC on both sides, pool-as-CR awkward; same blast radius as `sandboxes create`), admin still owns the MCP Deployment (two owners; proxy would redo it), operator mints investigation tokens (identity provider), operator mints MCP client tokens / stores a bearer on status, admin provides HMAC (internal MCP↔agent secret; ceremony with no admin-chosen value), admin applies client SA / `/mcp` ClusterRole / per-instance auth-delegator YAML (Q13), copy as-built 2× idle age-drain of unassigned pool pods (MCP-replica overshoot leftover; operator trims surplus immediately), enqueue every last-activity patch or custom `AddAfter` per bash (hammers the instance reconciler; a later `requeueAfter` reread is enough), MCP secret get/list/watch (would read other CRs’ kubeconfigs and HMAC in the same namespace), RBAC `resourceNames` or label selectors for dynamic session Secrets (API cannot), unbound operator `clusterrolebindings` verbs (Q13)._
...
- **`ownerRef` → CR:** MCP Deployment, Service, NPs, SAs (including client SA), Role/RoleBinding (including client Role), HMAC Secret, ConfigMap `cli-mcp-<name>-krp`, warm-pool pods. After the finalizer drops, Kubernetes GCs these. Do not wait for those objects in the finalizer. ClusterRoleBinding `cli-mcp-auth-delegator` has **no** instance `ownerRef`. Desired subjects omit `deletionTimestamp != nil`; the deleting instance’s finalizer still patches the CRB (required when it is the last instance).
...
1. **Auth-delegator is sidecar machinery, not admin policy.** kube-rbac-proxy cannot TokenReview / SubjectAccessReview without it. The operator already creates the sidecar and the MCP SA that needs the binding. There is no admin-chosen value. Per-CR sample YAML is ceremony, not consent (the cluster-admin already installed the operator).
2. **`/mcp` as a non-resource URL is not instance-scoped.** A ClusterRole grant on `/mcp` is cluster-wide. One client token that passes kube-rbac-proxy on instance `oc` also passes on instance `aws` if the caller can reach that Service. Per-instance client SAs do not isolate instances while authorization stays NonResourceAttributes.
3. **A named ClusterRoleBinding plus `bind` on `system:auth-delegator` only is not `cluster-admin`.** Kubernetes RBAC already stops a SA from binding a ClusterRole it cannot `bind`. Unrestricted `clusterrolebindings` verbs (list/watch/delete on all CRBs) are a different and still-rejected grant.

### Option A: Operator owns sidecar, per-instance client SA/Role, and one named auth-delegator ClusterRoleBinding

- **Operator owns:** kube-rbac-proxy sidecar (always; `RELATED_IMAGE_KUBE_RBAC_PROXY`); per-instance client SA `cli-mcp-<name>-client` (do **not** reuse MCP Role `cli-mcp-<name>`); namespaced Role/RoleBinding on fake subresource `climcpinstances/mcp` (`resourceNames` = CR name, verbs `get`/`create`/`delete`; do not grant `climcpinstances` without the subresource, or `create`/`delete` on `services`); ConfigMap `cli-mcp-<name>-krp` so kube-rbac-proxy uses ResourceAttributes for this CR (`--config-file=/etc/kube-rbac-proxy/config.yaml`; stamp ConfigMap RV on the pod template like HMAC); `status.clientServiceAccount` (always published; OpenAPI description carries the mint command; no token on status); ClusterRoleBinding `cli-mcp-auth-delegator` → built-in `system:auth-delegator`, subjects = MCP SAs of instances with `deletionTimestamp == nil` (**replace**, not merge; no instance labels as a selector contract). No dedicated proxy SA. No new CR. Operator does not create or modify `system:auth-delegator`.
- **Operator SA:** `bind` on `clusterroles` `resourceNames: [system:auth-delegator]` (quote the name in the kubebuilder marker); `create` on `clusterrolebindings` (unscoped — kube collection create has no name); get/update/patch on `clusterrolebindings` `resourceNames: [cli-mcp-auth-delegator]` (no `list`/`watch`/`delete`; Get by name from instance reconcile — OwnNamespace cache must not inform on all CRBs). Quiet instances still `requeueAfter` 5m (operator constant) so a deleted CRB is recreated without that watch. Empty subjects if no live instances; do not delete the CRB. No instance `ownerRef` on the CRB. Foreign `roleRef` on that name → `ChildrenNotReady` (do not adopt). Deleting instance’s finalizer still patches the CRB (required when it is the last instance). Conflict on `resourceVersion` → requeue. One operator install per cluster. Configmaps on `namespaced_role.yaml`; `Owns(ConfigMap)`. Distinct from CSV TokenReview already granted to the **operator** metrics sidecar.
- **Admin owns:** investigation kubeconfig, TLS on generic Kubernetes, SCC/PSA. Admin mints a TokenRequest against the status SA name. First-party GitOps drops `cluster-rbac.yaml` (`/mcp` ClusterRole + shared `cli-mcp-client` SA); keep `investigation.yaml`.

`--allow-paths` covers `/mcp`, `/metrics`, `/live`, `/health`, `/sessions`, `/sessions/*` (path allowlisting; ResourceAttributes replace the `/mcp` ClusterRole, not `--allow-paths`). All of those paths SAR the same fake subresource (HTTP GET→`get`, POST→`create`, DELETE→`delete`). Sidecar probes stay TCP; MCP `/live` `/health` are loopback exec and do not need the client Role. `/metrics` uses the same SAR (Prometheus extra RoleBinding / later `spec` subject). TLS Secret `cli-mcp-<name>-tls`: serving-cert on OpenShift, admin-provided on generic Kubernetes.

- **Pro:** Standing up an instance is CR + investigation Secret (and TLS off OpenShift). Client grant is instance-scoped. Operator ClusterRole is `bind` + unscoped CRB `create` + named get/update/patch, not `list`/`watch`/`delete` on all CRBs.
- **Con:** Shared CRB cannot use instance `ownerRef`; subjects are a replace-set and the finalizer must still patch on delete.

**Decision:** Option A — kube-rbac-proxy is a fixed part of the MCP Deployment. Operator owns the client SA/Role, krp ConfigMap, status SA name, and ClusterRoleBinding `cli-mcp-auth-delegator`. Admin mints the client token; operator does not. HOW: [MCP client authentication](cli-mcp-operator-design.md#mcp-client-authentication-q13).

_Considered and rejected: admin-applied client SA / `/mcp` ClusterRole / per-instance auth-delegator YAML (`nonResourceURLs` is cluster-wide; extra ceremony per CR), unrestricted operator `clusterrolebindings` verbs (including list/watch/delete on all CRBs), watching ClusterRoleBindings cluster-wide, operator-minted client tokens or a `kubernetes.io/service-account-token` Secret child, per-instance auth-delegator ClusterRoleBinding, dedicated kube-rbac-proxy SA, extra `spec` client subjects this phase (later additive), no kube-rbac-proxy (no TLS + SA-token front door; breaks loopback-only bind)._
...
2. **Every operator-managed namespaced child is present and matches spec** (SAs including client SA, Role/RoleBinding including client Role, Service, NetworkPolicies, sandbox SA, HMAC Secret, ConfigMap `cli-mcp-<name>-krp`; kube-rbac-proxy must mount that ConfigMap with RV stamped on the pod template). ClusterRoleBinding `cli-mcp-auth-delegator` exists with `roleRef` `system:auth-delegator` and includes this instance’s MCP SA while it is not deleting. CRB apply failure or foreign `roleRef` on that name → `ChildrenNotReady`. Always publish `status.clientServiceAccount`. No client token Secret is a Ready gate.
