...
- [ ] 3.3 Validate before listen: non-empty `--sandbox-image`; readable non-empty `--hmac-key-file`; `http` requires `--stateless` and loopback `--address`; `stdio` rejects `--stateless`
...
- [ ] 3.8 For HTTP transport: create `http.Server` with mux; start `ListenAndServe` in a goroutine; `select` on signal context **or** serve-error channel; treat non-`http.ErrServerClosed` as fatal (cancel ctx, return error); for stdio: `server.Run` with `StdioTransport`
- [ ] 3.9 On SIGTERM/SIGINT: cancel background ctx, `http.Server.Shutdown` with timeout (≥ max bash timeout + buffer, prefer 310s), exit cleanly
...
- [ ] 5.4 Confirm Cobra flags match design; HTTP requires `--stateless` + loopback address; both transports require image + hmac key file
- [ ] 5.5 Confirm graceful shutdown on SIGTERM and fatal exit on unexpected ListenAndServe errors
