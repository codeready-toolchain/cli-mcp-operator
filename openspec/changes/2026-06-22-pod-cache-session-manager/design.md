...
- SessionManager with `GetOrCreatePod` (label lookup, cache, create pod + auth Secret, wait for ready)
...
- SandboxConfig struct for all tunables (image, CPU/mem, idle timeout, HMAC key, warm pool size)
...
### Decision 2: HMAC-SHA256 deterministic token derivation
...
### Decision 3: Label-based pod discovery as source of truth
...
### Decision 4: Create auth Secret before pod
...
### Decision 5: Poll-based `waitForReady` with 60s deadline
...
### Decision 6: Background annotation update after ExecuteCommand
...
### Decision 7: SessionManager directly uses `net/http` for agent communication
...
### Decision 8: `CleanupStale` is caller-driven (no internal ticker)
...
→ **Mitigation:** `Invalidate()` is called on HTTP errors (agent unreachable), immediately clearing the stale entry and forcing rediscovery on the next request.
