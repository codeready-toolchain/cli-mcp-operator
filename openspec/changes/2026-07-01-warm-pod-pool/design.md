...
- `ReconcilePool()` goroutine (every 30s) that maintains the target number of unassigned pods
- Atomic claim flow: list unassigned → label patch with `resourceVersion` (first-writer-wins) → create auth Secret → `POST /assign` to agent → async replenishment
