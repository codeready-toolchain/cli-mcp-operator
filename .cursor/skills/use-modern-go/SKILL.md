...
- `t.Context()`: ALWAYS use `t.Context()` when a test function needs a context.
...
- `omitzero`: ALWAYS use `omitzero` (not `omitempty`) for time.Duration, time.Time, structs, slices, maps.
...
- `b.Loop()`: ALWAYS use `b.Loop()` for the main loop in benchmark functions.
...
- `wg.Go(fn)`: ALWAYS use `wg.Go()` when spawning goroutines with sync.WaitGroup.
...
- `errors.AsType[T](err)` — generic alternative to `errors.As(err, &target)`.
